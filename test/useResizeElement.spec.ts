...
describe('isNullThrowError', () => {
    it('should throw an error if the value is null or undefined', () => {
        ...
        expect(() => isNullThrowError(value, message)).toThrowError(
            UseResizeElementError(message)
        );
    });
...
describe('throwErrorIfElementIsInvalid', () => {
    describe('throw an error if', () => {
        ...
        it('ref.current.scrollHeight is null', () => {
            const eleRef = {
                current: { style: { height: '10px' }, scrollHeight: null },
            } as unknown as MutableRefObject<any>;
            callAndExpectThrownException(eleRef);
        });
        it('ref.current.style is null', () => {
            const eleRef = {
                current: { style: null, scrollHeight: null },
            } as unknown as MutableRefObject<any>;
            callAndExpectThrownException(eleRef);
        });
    ...
    it('should not throw an error if all of the conditions pass', () => {
        const eleRef = {
            current: { style: { height: 'test' }, scrollHeight: 35 },
        } as unknown as MutableRefObject<ResizableElementShape>;
...
describe('resizeElementPostValidation', () => {
    it('should set the height to 0px', () => {
        ...
        resizeElementPostValidation(ele, () => '10px', {
            min: 0,
            max: 1000,
            gapAtBottom: 7,
        });
